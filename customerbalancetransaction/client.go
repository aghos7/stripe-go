// Package customerbalancetransaction provides the /customer_balance_transactions APIs
...
// Client is used to invoke /customer_balance_transactions APIs.
type Client struct {
	B   stripe.Backend
	Key string
}
...
func (c Client) New(params *stripe.CustomerBalanceTransactionParams) (*stripe.CustomerBalanceTransaction, error) {
	...
	path := stripe.FormatURLPath("/v1/customers/%s/customer_balance_transactions", stripe.StringValue(params.Customer))
...
func (c Client) Get(id string, params *stripe.CustomerBalanceTransactionParams) (*stripe.CustomerBalanceTransaction, error) {
	...
	path := stripe.FormatURLPath("/v1/customers/%s/customer_balance_transactions/%s", stripe.StringValue(params.Customer), id)
	transaction := &stripe.CustomerBalanceTransaction{}
...
func (c Client) List(listParams *stripe.CustomerBalanceTransactionListParams) *Iter {
	path := stripe.FormatURLPath("/v1/customers/%s/customer_balance_transactions", stripe.StringValue(listParams.Customer))
